...
import net.ashley.sourcecubed.core.common.PlayerAPI;
...
public class SourcePlayer {
    ...
    public static boolean moveEntityWithHeading(Player player, ISourceCubedEntity squakeEntity, float sidemove, float upmove, float forwardmove)
    {
        ...
        if((player.getAbilities().flying || player.isFallFlying()) && player.getVehicle() == null)
            return false;
        else didQuakeMovement = quake_moveEntityWithHeading(player, sidemove, upmove, forwardmove);
    ...
    public static void beforeOnLivingUpdate(Player player) {
        ...
        if(setDidJumpThisTick != null) {
            try {setDidJumpThisTick.invoke(null, false);} catch(Exception e){}
        }
        ...
        if(setIsJumping != null) {
            try {setIsJumping.invoke(null, isJumping(player));} catch(Exception e) {}
        }
    }
    ...
    public static boolean moveRelative(Player player, ISourceCubedEntity squakeEntity, float sidemove, float upmove, float forwardmove, float friction) {
        if(!player.level().isClientSide)
            return false;
        ...
        float wishspeed = friction; wishspeed *= 2.15f;
    ...
    public static void afterJump(Player player) {
        ...
        quake_Jump(player);
        if(didLongJump) {
            quake_LongJump(player);
            clampVerticalVelocity(player, 0.55);
        }
        if(setDidJumpThisTick != null) {
            try {
    ...
    private static boolean didLongJump = true;
    private static long lastLongJumpTime = 0;
    private static long LONGJUMP_COOLDOWN_MS = Config.longJumpCooldown;
    private static void quake_LongJump(Player player) {
        ...
        if (currentTime - lastLongJumpTime < LONGJUMP_COOLDOWN_MS) return;
        ...
        double speed = Math.sqrt(Math.pow(current.x, 2) + Math.pow(current.z, 2));
        ...
        Vec3 boost = new Vec3(
                lookAngle.x * forwardBoost,
                verticalBoost,
                lookAngle.z * forwardBoost
        );
    ...
    private static void clampVerticalVelocity(Player player, double maxY) {
        ...
        if (currentY > maxY) {
            PlayerAPI.setMotionY(player, maxY);
        }
    }
    ...
    public static boolean quake_moveEntityWithHeading(Player player, float sidemove, float upmove, float forwardmove) {
        if(player.onClimbable()) {
            ...
            // air movement
                ...
                double sv_airaccelerate = Config.airAccelerate;
                quake_AirAccelerate(player, wishspeed, wishdir[0], wishdir[1], sv_airaccelerate);

                if(Config.sharkingEnabled && Config.sharkingSurfTension > 0.0D && isJumping(player) && PlayerAPI.getMotionY(player) < 0.0F) {
                    ...
                    if(isFallingIntoWater)
                        PlayerAPI.setMotionY(player, PlayerAPI.getMotionY(player) * Config.sharkingSurfTension);
                }
            }
    ...
    private static boolean quake_DoTrimp(Player player) {
        if(Config.trimpingEnabled && player.isShiftKeyDown() && (lastLongJumpTime <= 750)) {
            ...
            if(curspeed > movespeed) {
                ...
                PlayerAPI.setMotionY(player, PlayerAPI.getMotionY(player) + speedbonus * curspeed * Config.trimpMult);

                if(Config.trimpMult > 0) {
                    float mult = (float) (1.0f / Config.trimpMult);
                    double motionX = PlayerAPI.getMotionX(player), motionZ = PlayerAPI.getMotionZ(player);
                    motionX *= mult;
                    motionZ *= mult;
                    PlayerAPI.setMotionHoriz(player, motionX, motionZ);
    ...
    private static void quake_WaterMove(Player player, float sidemove, float upmove, float forwardmove) {
        ...
        if(!isSharking || curspeed < 0.078F) {
            ...
            if(curspeed > 0.09)
                quake_ApplyWaterFriction(player, Config.sharkingWaterFriction);
    ...
    private static void quake_AirAccelerate(Player player, float wishspeed, double wishX, double wishZ, double accel) {
        ...
        float maxAirAcceleration = (float) Config.maxAirAccelPerTick;
    ...
    private static void quake_ApplySoftCap(Player player, float movespeed) {
        float softCapPercent = Config.softCap;
        float softCapDegen = Config.softCapDegen;

        if(Config.uncappedBunnyhopEnabled) {
            softCapPercent = 1.0F;
            softCapDegen = 1.0F;
        }
    ...
    private static void quake_ApplyHardCap(Player player, float movespeed) {
        ...
        float hardCapPercent = Config.hardCap;
